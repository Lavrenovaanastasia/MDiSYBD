...
### 1) User - идентификатор пользователя
 ...
 • username vatchat, NOT NULL -  имя пользователя (не может быть пустым, должно быть уникальным);
...
• car: Car[1] - ссылка на запись в таблице "Car".  Это поле указывает на тип услуг,  количество которых  указано  в  таблице.  "[1]" означает, что  в  таблице  "DepartmentCar"  для  каждого отдела  указано  только  одно  количество  для  каждого типа  услуг
